...
fprintf(fID, '-------------------------------------------------------------------------------------------\n');
fprintf(fID, '%5s / %5s %10s   %9s   %8s   %5s   %7s   %4s   %8s   %8s   %8s\n', ...
             'rbar', 'sbar', 'f', 'g', 'time', 'iter', 'mean ls', 'mu0', 'mu', 'L_mu', 'status');
fprintf(fID, '-------------------------------------------------------------------------------------------\n');
...
for jj = 1:length(rbars)
    ...
    % print results
    fprintf('%4s / %4s   %10s     %9s    %8s     %5s   %7s   %4s   %8s  %8s   %8s\n', ...
             'rbar', 'sbar', 'obj', 'g', 'time', 'iter', 'mean ls', 'mu0', 'mu', 'L_mu', 'status');
    fprintf('%4.2g / %1d | %10.10f | %9.2e | %8.5f | %5d | %7.2f | %4.2g | %8.2e | %8.2e | %1d\n', ...
    ...
    fprintf(fID, '%4.2g / %1d | %10.10f | %9.2e | %8.5f | %5d | %7.2f | %4.2g | %8.2e | %8.2e | %1d\n', ...
        opt.rbar, opt.sbar, psi_temp, g_temp, time_temp, ...
        out_temp.k, mean(out_temp.ls(:,2),'all'), out_temp.mu0, out_temp.mu, out_temp.L_mu, out_temp.status);
